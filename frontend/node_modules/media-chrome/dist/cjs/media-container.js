...
template.innerHTML = `
  <style>
    ...
    ${""}
    :host([${Attributes.USER_INACTIVE}]:not([${import_constants.MediaUIAttributes.MEDIA_PAUSED}]):not([${import_constants.MediaUIAttributes.MEDIA_IS_AIRPLAYING}]):not([${import_constants.MediaUIAttributes.MEDIA_IS_CASTING}]):not([${Attributes.AUDIO}])) ::slotted(:not([slot=media]):not([${Attributes.NO_AUTOHIDE}])) {
      opacity: 0;
  ...
  constructor() {
        ...
        if (mutation.type === "childList") {
          ...
          if (media) {
            mutation.addedNodes.forEach((node) => {
              if (node === media && media !== __privateGet(this, _currentMedia)) {
                this.handleMediaUpdated(media).then((media2) => this.mediaSetCallback(media2));
              }
            });
          }
        }
    ...
    if (chainedSlot) {
      chainedSlot.addEventListener("slotchange", () => {
        ...
        if (this.media && this.media !== __privateGet(this, _currentMedia)) {
